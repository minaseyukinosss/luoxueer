...
const PlayerControls: React.FC<PlayerControlsProps> = ({ size = 'md' }) => {
  ...
  return (
    <div className={`flex items-center justify-center ${isSmall ? 'gap-3' : 'gap-6'}`}>
      ...
      <button
        onClick={nextSong}
        className={`flex items-center justify-center text-gray-400 hover:text-gray-800 transition-all active:scale-90 transform p-1.5 rounded-full hover:bg-gray-100/80 ${isSmall ? '' : ''}`}
        title="Next"
      >
