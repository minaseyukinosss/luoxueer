    ...
    <div className="h-full w-full flex flex-col md:flex-row md:items-center md:justify-center md:gap-16 lg:gap-24 py-4 md:py-0">
      ...
      <div className="flex-shrink-0 flex flex-col items-center md:items-end md:w-auto mb-6 md:mb-0 transition-all duration-500">
        ...
        <div className="relative group mb-6">
          ...
          <div className={`absolute inset-0 bg-black/20 blur-3xl rounded-full transform translate-y-4 transition-all duration-700 ${isPlaying ? 'opacity-100 scale-105' : 'opacity-60 scale-100'}`}></div>
          <div className={`absolute inset-0 bg-black/10 blur-2xl rounded-full transform translate-y-2 transition-all duration-700 ${isPlaying ? 'opacity-80' : 'opacity-40'}`}></div>

          {/* Tone Arm - Decorative */}
          <div className={`absolute top-[-10%] right-[-10%] w-24 h-32 z-20 pointer-events-none transition-transform duration-1000 origin-[70%_10%] ${isPlaying ? 'rotate-[25deg]' : 'rotate-0'}`}>
            <svg width="100%" height="100%" viewBox="0 0 100 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-lg">
              {/* Base */}
              <circle cx="70" cy="15" r="12" fill="#d1d5db" stroke="#9ca3af" strokeWidth="2" />
              <circle cx="70" cy="15" r="4" fill="#4b5563" />
              {/* Arm */}
              <path d="M70 15 L60 80 L35 95" stroke="#e5e7eb" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M70 15 L60 80 L35 95" stroke="#9ca3af" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="opacity-50" />
              {/* Head */}
              <rect x="25" y="90" width="15" height="20" rx="2" transform="rotate(30 32.5 100)" fill="#1f2937" />
              <rect x="28" y="92" width="10" height="16" rx="1" transform="rotate(30 33 100)" fill="#374151" />
            </svg>
          </div>
      ...
      <div className="flex-1 min-h-0 md:h-full overflow-hidden relative w-full max-w-xl flex flex-col justify-end md:justify-center">
        ...
        >
          <div className="flex flex-col items-center md:items-start py-[15%] md:py-[35%] space-y-3 md:space-y-4 px-4 md:px-0">
            {lyrics.map((line, index) => {
              ...
              return (
                <div
                  ...
                  className={`group relative transition-all duration-500 ease-out flex items-center justify-center md:justify-start w-full text-center md:text-left ${isActive ? 'opacity-100' : 'opacity-40 hover:opacity-80'
                    }`}
                >
                  <p
                    className={`font-medium transition-all duration-300 leading-relaxed tracking-wide cursor-default ${isActive
