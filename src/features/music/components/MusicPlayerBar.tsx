...
export default function MusicPlayerBar({ onOpenQueue }: MusicPlayerBarProps) {
  ...
  return (
    <>
      ...
      <div className="hidden md:flex w-[95%] max-w-[1200px] mx-auto mb-6 h-20 bg-white/80 backdrop-blur-xl rounded-[2rem] border border-white/50 items-center px-8 justify-between gap-6 z-50 relative shadow-[0_8px_32px_rgba(0,0,0,0.08)] transition-all duration-500 ease-out">
      ...
      <div className="md:hidden fixed bottom-6 left-4 right-4 z-50">
        <div
          className="bg-white/90 backdrop-blur-2xl rounded-[2rem] shadow-[0_8px_30px_rgba(0,0,0,0.15)] border border-white/60 p-2 pr-4 flex items-center gap-3 cursor-pointer ring-1 ring-black/5"
          onClick={onOpenQueue}
        >
          {/* Spinning Cover */}
          <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 shadow-md border border-gray-100 relative">
            <Image
              ...
              width={48}
              height={48}
              className={`w-full h-full object-cover ${isPlaying ? 'animate-spin-slow-mobile' : ''}`}
            />
